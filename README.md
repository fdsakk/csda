...
`stats build` combines both operations. The report contains aggregated player and weapon statistics, estimated Time to Damage (TTD), estimated Reaction Time, a 0–100 evidence score with two review tiers (`watch` = grey zone, `cheater` = not humanly reproducible over many games), and evidence rows pointing to demo rounds and ticks. The score fuses timing, precision and supporting performance without double-counting correlated metrics. It is intended to prioritize manual review; it is not an automatic cheating verdict.
